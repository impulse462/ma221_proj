...
class LSQRFramework():
    ...
    def solve(self, rhs, m, n, itnlim=0, damp=0.0, M=None, N=None, atol=1.0e-9,
        ...
        if beta > 0:
            ...
            if N is not None:
                ...
                v = Nv
            alpha = sqrt(np.vdot(v,Nv))   # norm(v)
...
if __name__ == '__main__':
    ...
    A = np.array([[0.5]]) #LinearOperator(1, 1, matvec=lambda u: u/2, symmetric=True)
    C = np.array([[0.333]]) #LinearOperator(1, 1, matvec=lambda v: v/3, symmetric=True)
    B = np.array([[1.0]]) #LinearOperator(1, 1, matvec=lambda x: x.copy(), symmetric=True)
    rhs = np.array([[2.0]])

    lsqr = LSQRFramework(B)
    lsqr.solve(rhs, M=A, N=C, damp=1.0, show=True, m=A.shape[0], n=A.shape[1])
